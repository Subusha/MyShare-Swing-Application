...
Connection con;  //Connection is provided facility to make connection between database
    PreparedStatement pst; //PreparedStatement provide ability to execute sql querys this is performed fast
...
//load data in to table
    ...
    // add,update,delete data from database(ManageSupplier is parent method)


    public ManageSupplier() {
        connect();
        tableload();
    btnadd.addActionListener(new ActionListener() {  //add suppliers to DB
        @Override
        public void actionPerformed(ActionEvent e) {
            ...
            try {
                ...
                if ((name.equals("")) || (Integer.parseInt(age) < 20) || (city.equals("")) || (itemname.equals("")) || (Integer.parseInt(quantity) < 0) || (Integer.parseInt(telephone) < 0)) {
                    ...
                    pst.executeUpdate();  // query eken wena wenas kam tika wenawa
        ...
        btnsearch.addActionListener(new ActionListener() {  // Search suppliers from DB
            @Override
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    if(rs.next()==true)  //next() eken karanne user input ekak ganna eka == true kiyanne hari id ekak user dunnoth pahala code kotasa execute wenawa
                    {
        ...
        btnupdate.addActionListener(new ActionListener() { //update supplier details in DB
            @Override
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    pst.setString(1, name);    //me ilakkam kiaynne uda query eke thiyana ? mark walata adalawa
                    pst.setString(2, age);
                    pst.setString(3, city);
                    pst.setString(4, itemname);
                    pst.setString(5, quantity);
        ...
        btndelete.addActionListener(new ActionListener() {  //delete supplier from DB
            @Override
            public void actionPerformed(ActionEvent e) {
